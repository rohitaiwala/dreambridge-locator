...
const createGroupSchema = z.object({
  ...
  groupId: z.string().min(3, "Group ID must be at least 3 characters").regex(/^[a-z0-9-]+$/, "Group ID can only contain lowercase letters, numbers and hyphens")
});
const JoinCommunity = () => {
  ...
  return <div className="min-h-screen" style={{
    backgroundColor: "#212A31"
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-2xl mx-auto bg-[#2E3944] rounded-lg shadow-xl p-6 my-[44px]">
            ...
            <TabsContent value="search" className="space-y-4">
              ...
              <Form {...searchForm}>
                <form onSubmit={searchForm.handleSubmit(onSearchSubmit)} className="space-y-4">
                  <FormField control={searchForm.control} name="searchQuery" render={({
                  field
                }) => <FormItem>
                        <FormLabel className="text-[#D3D9D4]">Community Name or ID</FormLabel>
                        <FormControl>
                          <Input placeholder="Enter community name or ID..." className="bg-[#1D252B] text-[#D3D9D4] border-[#124E66]" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>} />
                  <Button type="submit" className="w-full bg-[#124E66] text-[#D3D9D4] hover:bg-[#0D3F52]">
                    <Search className="mr-2 h-4 w-4" />
                    Search Communities
                  </Button>
                </form>
              ...
              <Form {...createForm}>
                <form onSubmit={createForm.handleSubmit(onCreateSubmit)} className="space-y-4">
                  ...
                  field
                        ...
                        <FormControl>
                          <Input placeholder="Enter community name..." className="bg-[#1D252B] text-[#D3D9D4] border-[#124E66]" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>} />
                  
                  <FormField control={createForm.control} name="description" render={({
                  field
                }) => <FormItem>
                        <FormLabel className="text-[#D3D9D4]">Description</FormLabel>
                        <FormControl>
                          <Input placeholder="Describe your community..." className="bg-[#1D252B] text-[#D3D9D4] border-[#124E66]" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>} />
                  
                  <FormField control={createForm.control} name="classId" render={({
                  field
                }) => <FormItem>
                        <FormLabel className="text-[#D3D9D4]">Class</FormLabel>
                        <FormControl>
                          <select className="w-full h-10 rounded-md border border-[#124E66] bg-[#1D252B] text-[#D3D9D4] px-3 py-2" {...field}>
                            <option value="">Select a class</option>
                            {classOptions.map(option => <option key={option.value} value={option.value}>
                                {option.label}
                              </option>)}
                          </select>
                        </FormControl>
                  ...
                  <FormField control={createForm.control} name="groupId" render={({
                  field
                }) => <FormItem>
                        <FormLabel className="text-[#D3D9D4]">Community ID</FormLabel>
                        <FormControl>
                          <Input placeholder="Create a unique ID (lowercase, numbers, hyphens)" className="bg-[#1D252B] text-[#D3D9D4] border-[#124E66]" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>} />
